...
│  │    • mcp__shabti__* (scheduler tools via IPC)                │   │
│  │                                                                │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
│   ├── shabti.log               # Host stdout
│   └── shabti.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.shabti.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'shabti-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your shabti installation
...
Shabti uses a hierarchical memory system based on CLAUDE.md files.
...
Shabti has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__shabti__schedule_task]
        {
...
The `shabti` MCP server is created dynamically per agent call with the current group's context.
...
Shabti runs as a single macOS launchd service.

### Startup Sequence

When Shabti starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned Shabti containers from previous runs
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/shabti.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/shabti.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.shabti.plist ~/Library/LaunchAgents/

# Start service
launchctl load ~/Library/LaunchAgents/com.shabti.plist

# Stop service
launchctl unload ~/Library/LaunchAgents/com.shabti.plist

# Check status
launchctl list | grep shabti

# View logs
tail -f logs/shabti.log
```
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep shabti` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; Shabti auto-starts container runtime but may fail |
...
- `logs/shabti.log` - stdout
- `logs/shabti.error.log` - stderr
