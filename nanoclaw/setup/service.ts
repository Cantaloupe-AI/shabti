...
function setupLaunchd(projectRoot: string, nodePath: string, homeDir: string): void {
  const plistPath = path.join(homeDir, 'Library', 'LaunchAgents', 'com.shabti.plist');
...
<dict>
    ...
    <string>${projectRoot}/logs/shabti.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/shabti.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.shabti');
  } catch {
...
/**
 * Kill any orphaned shabti node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned shabti processes');
  } catch {
...
function setupSystemd(projectRoot: string, nodePath: string, homeDir: string): void {
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/shabti.service';
    ...
    unitPath = path.join(unitDir, 'shabti.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=Shabti Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/shabti.log
StandardError=append:${projectRoot}/logs/shabti.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned shabti processes to avoid WhatsApp conflict errors
  ...
  try {
    execSync(`${systemctlPrefix} enable shabti`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start shabti`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active shabti`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(projectRoot: string, nodePath: string, homeDir: string): void {
  ...
  const wrapperPath = path.join(projectRoot, 'start-shabti.sh');
  const pidFile = path.join(projectRoot, 'shabti.pid');

  const lines = [
    '#!/bin/bash',
    '# start-shabti.sh — Start Shabti without systemd',
    ...
    '    echo "Stopping existing Shabti (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting Shabti..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/shabti.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/shabti.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "Shabti started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/shabti.log"`,
  ];
