...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.shabti')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.shabti'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active shabti`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
        const output = execSync(`${prefix} list-unit-files`, { encoding: 'utf-8' });
        if (output.includes('shabti')) {
          service = 'stopped';
        }
      } catch {
  ...
  if (fs.existsSync(path.join(homeDir, '.config', 'shabti', 'mount-allowlist.json'))) {
