...
export async function sendPoolMessage(
  ...
  try {
    ...
    const opts = { parse_mode: 'Markdown' as const, link_preview_options: { is_disabled: true } };
...
/**
 * Download a Telegram file and save it to the group's IPC input directory.
 * Returns the container-relative path (e.g., /workspace/ipc/input/photo_123.jpg).
 */
async function downloadTelegramFile(
  ...
  try {
    ...
    logger.info({ fileId, hostPath, containerPath }, 'Telegram file downloaded');
...
export class TelegramChannel implements Channel {
  ...
  async sendMessage(jid: string, text: string): Promise<void> {
    ...
    try {
      ...
      const opts = { parse_mode: 'Markdown' as const, link_preview_options: { is_disabled: true } };
      if (text.length <= MAX_LENGTH) {
        await this.bot.api.sendMessage(numericId, text, opts);
      } else {
        for (let i = 0; i < text.length; i += MAX_LENGTH) {
