...
async function runMatrixEntry(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'shabti-ci-'));
  ...
  try {
    ...
    // Initialize shabti dir
    execSync('npx tsx -e "import { initShabtiDir } from \'./skills-engine/index.js\'; initShabtiDir();"', {
      cwd: tmpDir,
      stdio: 'pipe',
      timeout: 30_000,
    });
