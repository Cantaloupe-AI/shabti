...
export function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'shabti-test-'));
}

export function setupShabtiDir(tmpDir: string): void {
  fs.mkdirSync(path.join(tmpDir, '.shabti', 'base', 'src'), { recursive: true });
  fs.mkdirSync(path.join(tmpDir, '.shabti', 'backup'), { recursive: true });
}
