...
describe('fetch-upstream.sh', () => {
  ...
  beforeEach(() => {
    ...
    upstreamBareDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'shabti-upstream-'),
    );
    ...
    const seedDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'shabti-seed-'),
    );
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: 'shabti', version: '2.0.0' }),
    );
    ...
    projectDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'shabti-project-'),
    );
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: 'shabti', version: '1.0.0' }),
    );
  ...
  it('uses existing upstream remote', () => {
    ...
    expect(status.TEMP_DIR).toMatch(/^\/tmp\/shabti-update-/);
  ...
  it('uses origin when it points to qwibitai/shabti', () => {
    // Set origin to a URL containing qwibitai/shabti
    execSync(
      `git remote add origin https://github.com/qwibitai/shabti.git`,
  ...
  it('adds upstream remote when none exists', { timeout: 15_000 }, () => {
    ...
    expect(remotes).toContain('qwibitai/shabti');
  });
