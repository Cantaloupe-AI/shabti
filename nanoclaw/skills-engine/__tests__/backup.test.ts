...
import { createTempDir, setupShabtiDir, cleanup } from './test-helpers.js';
...
describe('backup', () => {
  ...
  it('clearBackup removes backup directory', () => {
    ...
    const backupDir = path.join(tmpDir, '.shabti', 'backup');
  ...
  it('createBackup writes tombstone for non-existent files', () => {
    ...
    const tombstone = path.join(tmpDir, '.shabti', 'backup', 'src', 'newfile.ts.tombstone');
