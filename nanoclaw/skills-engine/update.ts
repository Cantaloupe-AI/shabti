...
import { BASE_DIR, SHABTI_DIR } from './constants.js';
  ...
  try {
    ...
    for (const relPath of preview.filesChanged) {
      ...
      const tmpCurrent = path.join(
        os.tmpdir(),
        `shabti-update-${crypto.randomUUID()}-${path.basename(relPath)}`,
      );
    ...
    const remapFile = path.join(newCorePath, '.shabti-meta', 'path_remap.yaml');
