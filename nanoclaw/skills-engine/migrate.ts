...
import { BASE_DIR, CUSTOM_DIR, SHABTI_DIR } from './constants.js';
import { initShabtiDir } from './init.js';
import { recordCustomModification } from './state.js';

export function initSkillsSystem(): void {
  initShabtiDir();
  console.log('Skills system initialized. .shabti/ directory created.');
}
...
export function migrateExisting(): void {
  ...
  try {
    ...
    if (diff.trim()) {
      ...
      console.log(
        'Custom modifications captured in .shabti/custom/migration.patch',
      );
    } else {
